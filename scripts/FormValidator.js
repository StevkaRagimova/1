export class FormValidator {
  ...
  disableCreateButton() {
    this.buttonElement.disabled = this._checkFormValidity() ? true : false;
    this.buttonElement.classList.toggle(this._config.inactiveButtonClass, this._checkFormValidity());
  }

  _toggleButtonState = () => {
    const isFormInvalid = this._checkFormValidity();
    this.buttonElement.disabled = isFormInvalid;
    this.buttonElement.classList.toggle(this._config.inactiveButtonClass, isFormInvalid);
  }
  ...
  _showErrorNotice(selectedInput) {
    const errorNotice = this._formElement.querySelector(`.${selectedInput.id}-error`);
